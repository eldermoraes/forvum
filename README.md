...
- **Tools, gated** — filesystem, [web fetch](forvum-tools-web), [shell](forvum-tools-shell), a [headless browser](forvum-tools-browser), a [code sandbox](forvum-tools-sandbox), an [MCP bridge](forvum-tools-mcp-bridge), and an explicit [memory](forvum-tools-memory) save/recall — each behind permission scopes, role-based access, and an interactive approval gate for sensitive calls.
...
Releases also ship a CycloneDX SBOM (`forvum-<version>-sbom.cdx.json`) and an image SBOM; the full
