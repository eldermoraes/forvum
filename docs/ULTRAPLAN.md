...
18. **QA scenario suite.** `forvum qa suite` / `forvum qa <channel>` runs a scripted scenario pack against the running assistant; fails by default on any unverified scenario. The scenario pack ships in the release and the suite is a CI gate. *Realized (P2-QA):* `forvum-app` `QaCommand` (registered in `RootCommand`; NOT a `CommandMode` one-shot — it runs turns, so it needs Flyway/Panache) loads the bundled `qa/scenarios.json` classpath resource (override with `--pack <file>`; hinted into the native image via `quarkus.native.resources.includes=qa/**`) and runs each scenario through the real turn path (the SDK `ChannelTurnDriver`/`TurnService`, like `forvum ask`). **Scenario format** (minimal, deliberately shared with the eval harness — item under #58): a pack is `{ "scenarios": [ { "id", "channel", "input", "expect": { "match": "exact|contains|regex", "value" } } ] }`. CI needs NO live inference: the seeded home pins `main` to a bundled, network-free `echo` `ModelProvider` (extension id `echo`, INERT unless pinned — like the bundled Qdrant memory provider) whose reply is `"echo: <input>"`, so an `expect` is deterministic on the JVM jar AND the native binary. Fails-by-default: an empty/absent pack or a channel target matching no scenario exits 1 (never a vacuous pass); any failed scenario exits 1; logs go to stderr, the summary to stdout. The CI gate is a separate `.github/workflows/qa.yml` (JVM fast-jar + native binary), plus the in-suite `QaSuiteNativeIT` running in `ci.yml`'s default native leg.
