...
- **Tools, gated** — filesystem, [web fetch](forvum-tools-web), [shell](forvum-tools-shell), a [headless browser](forvum-tools-browser), a [code sandbox](forvum-tools-sandbox), an [MCP bridge](forvum-tools-mcp-bridge), an explicit [memory](forvum-tools-memory) save/recall, [text-to-speech](forvum-tools-tts), [image + PDF analysis](forvum-tools-multimodal), and skill invocation (`skill.invoke` / `skill.list`, pulling a named `skills/` prompt template into a turn) — each behind permission scopes, role-based access, and an interactive approval gate for sensitive calls.
...
- **Semantic + episodic memory** — the bundled local SQLite `LocalMemoryProvider` writes durable facts and real episodes off-turn and reads them back on the next turn **by default** (identity/agent-scoped, no external service required); a pluggable [Qdrant](forvum-provider-memory-qdrant) backend is an opt-in alternative selected only when configured. Inspect and manage the store directly with `forvum memory query` / `search` / `reindex` / `forget`.
- **Observable by design** — every turn, tool call, and fallback is written to the ledger; OpenTelemetry spans and a CAPR dashboard are one config flag away (the per-turn CAPR *judge verdict* shipped with [#195](https://github.com/eldermoraes/forvum/issues/195): an opt-in, off-critical-path LLM-as-judge replaces the v0.5 placeholder).
...
`tts.speak`, `image.analyze`/`pdf.analyze`, `skill.invoke`/`skill.list`, and `mcp.*` are compiled in but
**belt-excluded by default** — add an id to `allowedTools`
