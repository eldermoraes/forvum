...
/**
 * {@code EnvelopeProcessor.render} maps each {@code AgentEvent} to the text the Signal sender receives.
...
class SignalRenderTest {
    ...
    void tokenDeltaRendersItsText() {
        assertEquals("hi", EnvelopeProcessor.render(new TokenDelta(T, "hi", MODEL)));
    }
    ...
    void errorEventRendersItsMessage() {
        ...
        assertEquals("boom", EnvelopeProcessor.render(error));
    }

    @Test
    void doneRendersEmpty() {
        assertEquals("", EnvelopeProcessor.render(new Done(T, UUID.randomUUID(), "reply")));
    }

    @Test
    void toolInvokedRendersEmpty() {
        assertEquals("", EnvelopeProcessor.render(new ToolInvoked(T, 1L, "fs.read", "{}")));
    }

    @Test
    void toolResultRendersEmpty() {
        assertEquals("", EnvelopeProcessor.render(new ToolResult(T, 1L, "ok", InvocationStatus.OK, 5L)));
    }

    @Test
    void fallbackTriggeredRendersEmpty() {
        assertEquals("",
                EnvelopeProcessor.render(
