...
- **Tools, gated** — filesystem, [web fetch](forvum-tools-web), [shell](forvum-tools-shell), a [headless browser](forvum-tools-browser), a [code sandbox](forvum-tools-sandbox), an [MCP bridge](forvum-tools-mcp-bridge), an explicit [memory](forvum-tools-memory) save/recall, [text-to-speech](forvum-tools-tts), and [image + PDF analysis](forvum-tools-multimodal) — each behind permission scopes, role-based access, and an interactive approval gate for sensitive calls.
