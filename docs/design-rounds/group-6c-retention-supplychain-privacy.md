...
|---|---|---|---|---|
| 1 | Model provider calls | the prompt window: messages, retrieved memory, tool results | operator configures the provider + key (M9–M12); local Ollama is the scaffolded default (`init` pins `ollama:qwen3:1.7b`) | on (the product's purpose) |
