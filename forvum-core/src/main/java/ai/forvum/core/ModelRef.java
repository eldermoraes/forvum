...
/**
 * Value object identifying an LLM as {@code provider:model}.
 *
 * <p>The provider is case-folded to lowercase; the model is case-preserved (provider model
 * identifiers are case-sensitive at the API level). {@link #parse(String)} splits on the FIRST
 * colon only so provider-specific tags that themselves contain colons (e.g. Ollama's
 * {@code qwen3:1.7b}) survive the round-trip.
 */
