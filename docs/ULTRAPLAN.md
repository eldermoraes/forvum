...
- **`forvum-channel-telegram`** — long-poll bot (webhook available as an opt-in alternative); uses a blocking REST client on a virtual thread (`quarkus-rest-client` with `@RunOnVirtualThread`) per the virtual-threads-first principle (§3.8).
...
- **Telegram.** Long-poll mode via a blocking `quarkus-rest-client` on a virtual thread (`@RunOnVirtualThread`) calling the Telegram Bot API. Webhook mode is an opt-in variant for deployments behind a public URL.
