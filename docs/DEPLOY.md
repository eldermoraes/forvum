...
- **Cloud provider (no GPU on the VPS).** Put the key in an `0600` env file (read by the service):
  ...
  ```bash
  sudo -u forvum sed -i 's#"ollama:qwen3:1.7b"#"anthropic:claude-sonnet-4-5"#' /var/lib/forvum/agents/main.json
  ```
...
- **Local Ollama on the VPS (needs enough RAM/CPU; a GPU helps).**
  ...
  The default `agents/main.json` already points at `ollama:qwen3:1.7b`, so no edit is needed.
