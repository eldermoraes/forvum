...
8. **#169** ✅ `[HIGH]` `feat(budget)`: enforce cost & tool budgets at runtime — **unblocks #177**. *Done:* `costBudget` parsing un-deferred (DR-8 DP-6: day-only file windows, timezone at parse); the Decision-8 pre-call gate rides `FallbackChatModel` (checked per attempt, `FallbackTriggered(COST_BUDGET)` + `BudgetExhaustedException` short-circuit) with the meter's day aggregation scoped per agent (Decision 10); `toolBudget` is a per-turn atomic grant counter consumed by `ToolExecutor` after the belt/RBAC/approval gates (denied calls never consume) and audited `denied` on exhaustion; cron fires resolve budget-gated; spawn gains the `CostBudget` override + the `SessionWindow` inheritance guard; `TurnService` surfaces `code=budget_exhausted`. The graph `MAX_ROUNDS` stays the separate liveness backstop.
9. **#172** ✅ `security(output-guard)`: sanitize `ErrorEvent` before channel delivery — **prerequisite for #173**; keep the error contract compatible with #165. *Done:* a single `emitError` seam in `TurnService` sanitizes all five `ErrorEvent` sites — the untrusted `turn_failed` message is genericized (category + root-cause class name + curated connection hint + a `turnId` correlation `ref`, never raw exception text); curated config errors pass through the `SecretRedactor`; the channel event keeps the exception class name (safe, for telemetry) but nulls the stack trace and the full detail is logged (redacted) keyed by `turnId`. All ten renderers read `.message()` verbatim, so no channel changed.
10. **#173** ✅ `security(persistence)`: owner-only (0700/0600) permissions for runtime state — consumes #172's sanitized error contract. *Done:* `StateDirInitializer` enforces the `InitCommand` recipe on **every** boot — `state/` created `0700` independent of umask, a pre-existing world-accessible dir repaired, and the SQLite database + WAL/SHM sidecars tightened to `0600` after migration (wired in `PersistenceBootstrap.onStart`, covering CLI/`ask`/server/cron). Fail policy is **repair-and-warn** (option A: never blocks the boot, preserving the M4 graceful-boot contract + the CI native smoke + K8s/PVC); a **symlinked `state/` is hard-rejected** (path substitution). Non-POSIX warns once; permission-failure logs omit the absolute state path ([6c-DP-14]). Proven on both targets: JVM `StatePermissionsIT` (real Flyway/Hibernate → `0700`/`0600`) + default-leg `StatePermissionsNativeIT` (binary boot).
11. **#176** ✅ `fix(context)`: bound compression-failure fallbacks (don't reinsert raw text) — fixes the **Compress** pillar's fail-open path; **#175 consumes this contract**. *Done:* a shared `BoundedCompressor` policy at both the retrieved-memory and worker-digest boundaries truncates a failed compression to the threshold + a fixed marker (never raw), classifies the failure (timeout/exception/blank/invalid/over-limit/input-over-limit) into a safe content-free log, skips the model above `threshold×4`, and re-throws `BudgetExhaustedException` (#169).
12. **#177** ✅ `fix(spawn)`: retire ephemeral workers & release `@AgentScoped` state — fixes an **Isolate** leak; depends on #169, shares a primitive with #178. *Done:* the supervisor `run()` `finally` retires every worker the turn spawned via `AgentRegistry.retire` (unregister + `@AgentScoped` context destroy through the ArC-registered `AgentContext`, reachable directly via `Arc.getContexts(AgentScoped.class)`), on EVERY exit path (success/model-fail/tool-fail/timeout/budget/interrupt), per-worker try/catch so a cleanup failure never masks the turn. Each worker gets a UNIQUE runtime id (`EphemeralAgentId`, `<label>~<uuid8>`) — never the model's raw childId — so a spawn can never collide with a persistent agent or another worker; live ids tracked in an `ephemeralIds` `newKeySet()` that is both the "never retire a persistent agent" guarantee and the `activeWorkerCount()` gauge (`cleanupFailureCount()` the other). ledger/task history survives teardown. 1000-cycle + concurrent race + native-safe (`Arc` API).
13. **#178** ✅ `fix(config)`: make agent hot-reload atomic & capability-safe — relates to #167/#168 (reliable role/tool revocation). *Done:* each registry entry is an immutable versioned `LiveAgent(generation, spec)`; a turn leases ONE generation at entry and binds it (`CURRENT_AGENT_SPEC` ScopedValue) so persona/scopes/model/belt all resolve the same frozen generation even under a mid-turn reload (enforce-iff-bound, the P2-11 pattern — all existing call-sites became coherent with no per-site edit). `AgentToolBelt`'s cache is dropped (the belt is recomputed from the lease per turn — the capability-safety fix), and `onConfigChange` publishes atomically (`specs.replace`, the ToolRegistry volatile-swap precedent): an invalid/half-written file keeps the last-known-good and is counted, never widening; DELETED unregisters + `destroyScope`s so a new lease is rejected while in-flight turns finish on their leased snapshot. No refcount/drain needed (the derived belt lives in the GC-per-turn snapshot, so context teardown never disrupts a lease); monotonic generations are ABA-safe. Proven JVM + native (the [M4] WatchService event-fire path).
14. **#171** ✅ `security(plugins)`: strict artifact checksums + owner-only plugin install — build-side later covered by #174. *Done:* `MavenPluginResolver` sets `CHECKSUM_POLICY_FAIL` on BOTH the session (global override) and every remote's release + snapshot policy, so a missing OR mismatched checksum aborts resolution and leaves no loadable JAR (the local `~/.m2` cache resolves without re-verification, inside the install-act trust boundary); the repository URL is restricted to a `{https, file}` scheme allowlist (plaintext `http://` rejected) and resolution-failure diagnostics redact `://user:secret@` credentials. A package-private `PluginArtifactInstaller` stages the JAR owner-only (`0700`/`0600` on POSIX) via an atomic temp-file + `ATOMIC_MOVE` (never a partial JAR; a failed install never alters the previously installed valid plugin), rejecting a symlinked `plugins/`/target and — unlike #173's boot-time repair-and-warn — **failing** the install (fail-closed, exit 1) if a pre-existing loose dir cannot be tightened. PGP signatures stay the documented DR-6b deferral.
...
21. **#184** ✅ ▸ `[HIGH]` `feat(tools)`: usable default tool belt + `forvum tools` discoverability — **cheapest, highest impact**: shell/web/browser/mcp tools already exist but the default belt is `fs.*`-only, so they silently no-op. #192 and #194 compose on it. *Done:* the `init` scaffold widens `DEFAULT_ALLOWED_TOOLS` to `fs.*`/`web.fetch`/`web.search`/`memory.*` AND wires `identityId: "default"` (without it a fresh home resolves anonymous and `scopeVisibleBelt` filters the WHOLE belt); `shell.exec`/`sandbox.run`/`browser.*`/`tts.speak`/`mcp.*` stay belt-excluded. New `forvum tools` one-shot lists every compiled-in tool (scope, confirm, owner, belt, readiness) gathered from `Instance<ToolProvider>` (skips the MCP-bridge connect, P2-13); a `ToolProvider.configGaps()` SDK default keeps each tool's "not configured" hint in its owning module; `forvum doctor` warns on a belted-but-unconfigured tool and on a belt dead-by-anonymity (upgraded installs). Zero new reflection surface; a `ToolsNativeIT` proves the picocli one-shot + provider discovery in the image.
22. **#192** ✅ ▸ `feat(tools-web)`: pluggable web-search backend SPI + keyless default (DuckDuckGo/SearXNG) — makes the `web.search` that #184 belts work out of the box; removes the Brave-key wall. *Done:* a module-internal `WebSearchBackend` seam (NOT a public SPI) selects the backend from `tools/web.json`'s `"backend"`; the keyless **DuckDuckGo** HTML backend is the DEFAULT (results with no API key), **Brave** is the keyed option (room for Tavily/Google/SearXNG). Precedence: an explicit `"backend"` wins, else a `braveApiKey` selects Brave, else DuckDuckGo. Zero new reflection surface (HTML string-parse, no new DTO); the DDG egress reuses the module's `EgressGuard`+`HttpFetcher` (strict, browser-UA on the search request only). Fixture-driven parser + backend-precedence + degrade-path unit tests, the re-pointed wiring IT (seeds `backend=brave` no-key so CI stays offline), and a `@Tag("live")` keyless smoke.
...
24. **#185** ✅ ▸ `[HIGH]` `feat(tools-multimodal)`: image + PDF analysis tools — a **real capability gap** (the input path is text-only today); new `MEDIA_ANALYZE` scope. *Done:* `forvum-tools-multimodal` contributes `image.analyze([paths],[prompt])` / `pdf.analyze(path,[prompt])` (both `MEDIA_ANALYZE`, `userConfirmRequired=false`), routing workspace media through a Resolution-B `MediaAnalysis` seam (engine `EngineMediaAnalysis` sole implementor) that resolves the vision model (`tools/multimodal.json` `model` override else the leased agent primary) and drives the budget-gated/ledgered `LlmSelector` path. Images → `ImageContent` (all providers); `pdf.analyze` dual-path: native `PdfFileContent` (anthropic/google/openai) else PDFBox text-layer extraction (no OCR) in a closing-tag-neutralized data block. PDFBox 3.0.8 adopted via the [#124] protocol (no bundled metadata / no ServiceLoader / BC excluded → encrypted rejected; a hand-authored `resource-config.json` for the AFM/glyphlist/cmap resources; `commons-logging` bridged to jboss-logging). `MEDIA_ANALYZE` appended as `PermissionScope` #11 (`cron` role excluded). No `configGaps` (functional with no config). Live native vision/PDF turn proven by a local `-Pnative` build (Layer-3 native discipline), not a per-PR CI job.

## Wave 6 — Context-engineering differentiator arc *(the north star's SOTA edge)*

25. **#190** ✅ ▸ `feat(tools)`: structured planning `update_plan` tool — **Write** pillar; strongest single driver of multi-step coherence. Plan in the session scratchpad with a compaction `block_type`. *Done:* an engine-resident built-in offered alongside the belt like `spawn_worker` (hand-built `PLAN_SPEC`, no `PermissionScope`, intercepted at the top of the graph's `runTool` after the replay short-circuit; zero `ToolCallBridge`/SDK changes). Full-plan-replace calls are validated engine-side (`PlanFormat`: ≥1 step, ≤1 `in_progress`, caps 50/300/8 000 — over-cap rejects, never truncates) and persisted append-only as ONE rendered-text `messages` row (`role='tool'`, code-only `BlockType.PLAN` — no migration, zero new reflection). The live (newest) plan is injected once at turn entry as a closing-tag-neutralized `<current_plan>` data block before the user's question; compaction retains the region's newest plan verbatim and strips superseded ones as orphans (never summarized). `GraphState` stays serialization-clean (R6).
