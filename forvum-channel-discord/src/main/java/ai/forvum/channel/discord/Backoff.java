...
/**
 * A pure, lock-free exponential-backoff counter for the Discord gateway reconnect loop: each
 * {@link #nextDelayMillis()} doubles the delay (starting at {@code initialMillis}) up to a {@code maxMillis}
 * cap, and {@link #reset()} returns to the initial delay (called on a successful READY so a healthy session
 * restarts the schedule). No clock, no IO, no {@code synchronized} — backed by a single {@link AtomicLong}
 * so the inbound-frame virtual thread (READY → reset) and the reconnect virtual thread (next delay) can
...
final class Backoff {
    ...
    /** Return the schedule to its initial delay (called on a successful READY). */
    void reset() {
