...
**Goal:** Ship Forvum — a local-first, open-source personal AI assistant in Java 25 / Quarkus 3.33.x LTS / LangChain4j 1.16.2 (via quarkus-langchain4j 1.11.0) / LangGraph4j 1.8.17 — that first reaches, then surpasses, OpenClaw on features and engineering quality.
...
**Tech Stack:** Java 25 · Maven · Quarkus 3.33.x LTS · Quarkiverse `quarkus-langchain4j-*` 1.11.0 (GA) · LangChain4j 1.16.2 (transitive via quarkus-langchain4j 1.11.0) · LangGraph4j 1.8.17 · Xerial SQLite JDBC · Hibernate ORM + Panache + Flyway · TamboUI 0.3.0 (as-built: headless `Buffer` render only, NO terminal backend — both jline3/panama fail the GraalVM 25 native build; see §3.5 M15) · Quarkus WebSockets Next · Quarkus Scheduler · OpenTelemetry · GraalVM for JDK 25 (Community Edition 25+); native builds use Mandrel 25.0.x-Final as the Quarkus-preferred `native-image` distribution · JaCoCo · GitHub Actions.
...
**Source-of-truth precedence & as-built status (read this first).** This document is the **normative, frozen design intent** and the architectural source of truth; `CLAUDE.md` defers to it and `docs/CONTEXT-ENGINEERING-MAPPING.md` maps it onto the four pillars. Precedence, highest to lowest: **(1)** the running code on `main` / the `v0.5.0` tag (*as-built reality*) · **(2)** this ULTRAPLAN (*normative design intent*) · **(3)** `docs/IMPLEMENTATION-ORDER.md` (*live roadmap status*) · **(4)** `docs/ISSUES.md` (*historical per-step proposals + as-built amendments*). A statement here describes **design intent**; where the runtime diverges, an inline *(As-built: … → #NNN)* note flags the gap and links the open tracking issue — absent such a note, assume the text matches `v0.5.0` behavior. **Status:** the whole M1–M20 roadmap and Phase-2 (v0.5, OpenClaw parity) have **shipped** (released as `v0.5.0`); Phase-3 (v1.0+) is next (§7.3). Known as-built gaps in shipped v0.5, each tracked by an open issue: spawn `@AgentScoped` leak (#177) and Web-channel/dashboard auth (#165). The compression-failure fallback is now bounded (#176), the three-tier Write/Select memory loop is now wired into normal turns (#175), and the placeholder CAPR verdict has been replaced by the real (opt-in) per-turn judge (#195).
...
- **`forvum-tools-web`** — `web.fetch` (`WEB_FETCH`, arbitrary-URL fetch over `java.net.http`) and `web.search` (`WEB_SEARCH`, a pluggable backend). Both are READ-only outbound HTTP — neither is in the #39 `USER_CONFIRM_REQUIRED` gate; both sit behind the belt + P2-11 RBAC scope gates. `web.fetch` carries a self-contained `EgressGuard` SSRF policy (block loopback/link-local/private by default, `allowPrivateNetwork` opt-in); the shared engine egress decorator (§1.1) is deferred. `web.search` selects a backend via a module-internal `WebSearchBackend` seam (NOT a public SPI) from `tools/web.json`'s `"backend"`: the keyless **DuckDuckGo** HTML backend is the DEFAULT (so search works with no API key — its egress reuses the same `EgressGuard`+`HttpFetcher`, always strict since the host is fixed public), and **Brave** is the keyed option (room for Tavily/Google/SearXNG). Precedence: an explicit `"backend"` wins, else a `braveApiKey` selects Brave, else DuckDuckGo (#192). Landed in PR-6; keyless default in #192.
...
- **Web.** Quarkus WebSockets Next for bidirectional streaming. The initial UI is a minimal hand-written HTML/JS page shipped as classpath static resources. *(As-built: the Web channel and the approval/CAPR dashboards ship with NO built-in authentication — bind to localhost and front with an authenticating reverse proxy; operator-grade auth for the dashboards and the chat socket is tracked by #165, in progress.)* A natural evolution path is Qute templates plus HTMX fragments for richer server-driven UI without adopting a full SPA toolchain.
...
- **OpenTelemetry** is wired via `quarkus-opentelemetry` (delivered by P2-15 #40 — the spans AND the export path together). The §3.6 OTel baseline below — four span kinds, each carrying a `thread.is_virtual` boolean attribute (§3.8) — is created on every turn, but the OTel SDK is **off by default**: it (and the OTLP exporter) activates only when `OTEL_EXPORTER_OTLP_ENDPOINT` is set (zero-config for Honeycomb/Tempo/Datadog), so a default run pays zero telemetry overhead and skips OTel resource detection. The engine defines four span kinds (the four turn-path seams):
  - `forvum.agent.turn` — one per inbound user message (`Agent.respond`); attributes include `forvum.agent.id`, `forvum.session.id`, `thread.is_virtual`.
  - `forvum.llm.call` — one per model invocation (`FallbackChatModel.chat`, covering any fallback hops); attributes include `forvum.model`, `thread.is_virtual`, and on a fallback `forvum.fallback`/`forvum.model.used`. The richer `tokens_in`/`tokens_out`/`cost_usd`/`latency_ms` live in the `provider_calls` ledger; folding them onto the span is a documented refinement.
  - `forvum.tool.call` — one per tool execution (`ToolExecutor.execute`); attributes include `forvum.tool.name`, `forvum.agent.id`, `thread.is_virtual`.
  - `forvum.graph.run` — one per supervisor-graph run (`SupervisorGraph.run`). A spawned worker's spans nest under it via OTel context propagation across the worker virtual-thread fan-out. (Per-LangGraph4j-node spans are a later refinement; v0.1 ships one span per graph run, matching the ratified four-seam baseline.)
...
| quarkus-langchain4j (Quarkiverse) | 1.11.0 GA (import `quarkus-langchain4j-bom:1.11.0`; aligned to Quarkus 3.33.2) | imported BOM — single AI-classpath governor |
| LangChain4j core | 1.16.2 (transitive via qlc4j 1.11.0) | not pinned independently |
...
The baseline adopts `quarkus-langchain4j` 1.11.0 GA (brings LangChain4j core 1.16.2, targets Quarkus 3.33.2), comfortably above the original LangChain4j ≥ 1.15.1 floor; the `maxSequentialToolsInvocations` → `maxToolCallingRounds` rename applies on the 1.15.x+ surface (§5.5).
...
- **`config.json`** holds cross-cutting settings: the default fallback chain for agents that do not declare their own, the log level, the enabled-channel set, and the embedding model used for semantic memory. It never holds secrets — provider API keys live owner-only (`0600`) under `state/credentials/<provider>`, written by `forvum provider add` (P2-10; the `CopilotCredentials` recipe generalized — OpenClaw file-based parity). A native platform keychain (macOS Keychain, Secret Service on Linux, Windows Credential Manager) is a documented fast-follow.
...
- **`skills/<skill>.md`** is a named prompt template with an optional leading `---`-fenced **JSON** front-matter (`{ "name", "description", "inputSchema" }`) declaring its input schema — the same v0.5-subset `OutputSchemaValidator` shape used for agent output schemas (one validator, no parallel schema); a skill with no front-matter is valid (the whole file is the template). `forvum skill install <url>` (P2-7 #32) downloads a skill from a URL (http/https/file — a git-raw URL or gist), validates it through the real `SkillReader` parser (a malformed front-matter or `inputSchema` is rejected before the file lands — the doctor-lesson "validate through the real reader"), and writes it owner-only (`0600`); `skills/` is hot-loaded (`ConfigWatcher`). Agents invoke skills by name through the engine-resident `SkillToolProvider` — the model-facing `skill.invoke` / `skill.list` tools (skills ARE tools; #191), belt-gated by the `SKILL_INVOKE` scope. `skill.invoke(name, args)` validates the arguments against the declared `inputSchema` (the same `OutputSchemaValidator`) BEFORE a literal single-pass `{{key}}` expansion, caps the expanded template at 32 000 chars, and feeds it back into the invoking agent's turn as a tool result; `skill.list` returns a bounded catalog for discovery. The template is read at invoke time (a `skills/` edit is visible on the next call — no cache), and any tool call it induces still crosses the caller's belt → RBAC → approval → budget. Skills are globally visible to any agent allowed to call the skill tools — they are not per-agent.
...
- **`channels/<channelId>.json`** is per-channel configuration. Channels that need secrets (e.g. the Telegram bot token) read them from the channel's own owner-only (`0600`) `channels/<channelId>.json` at channel start; a native platform keychain is a documented fast-follow.
...
- **`SessionWindow` inheritance without override is prohibited at spawn time.** A parent `CostBudget` with `SessionWindow` points at parent's `(sessionId, agentId)`; silently inheriting it into a child would cause the child's calls to be invisible to its own SUM, yielding an effectively unbounded budget with no warning. The spawn path throws `SpawnConfigurationException` in that case, carrying parent/child agent IDs and an educational message pointing the operator at the override mechanism. Document-as-degenerate and smart-inheritance (auto-reconstruct) were both rejected in favor of spawn-boundary surfacing. (Decision 10) *(As-built, #169: the guard is activated but is a defensive spawn-time safeguard for the programmatic 4-arg `spawn` override — it is unreachable from file config, since `AgentSpecReader` rejects a file-declared `"session"` window (§4.3.8, DR-8 DP-6), so no file-parsed agent carries a `SessionWindow` budget to inherit. In the M18 supervisor graph the only production spawn path is the `spawn_worker` fan-out, which renders this exception as a model-visible tool result and continues the turn — like every spawn failure — rather than the `spawn_invalid_config` terminal `ErrorEvent`; that terminal surface has no M18 spawn path that escapes to it.)*
...
- **`roles` is an agent-level scope CAP (intersection), never a grant.** Effective scopes = caller scopes ∩ the union of the agent roles' scope sets (via `RoleRegistry`), computed at the two existing `CURRENT_EFFECTIVE_SCOPES` bind sites (`TurnService.dispatch`, `CronScheduler.fire`); absent/empty = no cap. The spec names roles, not raw scopes — role-sets live above the `PermissionScope` enum (§4.3.4); a cap-only field can never escalate, mirroring the `allowedTools` belt philosophy. **Enforced as-built (#167):** the cap is `RoleRegistry.capScopes(callerScopes, persona.roles())` (NOT a reuse of `effectiveScopes`, whose empty-list branch is the permissive `default-user` — that would widen an uncapped agent); the capped set is the single value bound, so tool discovery (the model-facing belt is scope-filtered in `SupervisorGraph`, defense-in-depth) AND execution-time authorization observe one consistent set; a named-but-undefined role (on the identity OR the agent) fails CLOSED with an actionable config error — distinct from the empty-list no-cap — which `ConfigDoctor` also flags proactively. (DR-8 DP-8)
...
- **`spawn_worker`** — materializes a sub-agent via `AgentRegistry.spawn(parentId, spec)` with a narrowed tool belt and a child `CostBudget`. The sub-agent runs its own mini-graph inside its own `@AgentScoped` context. *(As-built: ephemeral spawned workers currently leak their `@AgentScoped` state instead of releasing it at turn end → #177.)*
...
- **Vetoed dependencies.** Any library that relies on `sun.misc.Unsafe`, runtime bytecode generation (CGLib, Javassist at runtime), or un-hinted reflection is excluded from the compile classpath via `<exclusions>` in `forvum-bom`. A CI check (`.github/native-discipline.sh`, X1/#67) greps every module's `src/main` for banned imports (`sun.misc.Unsafe`, `net.sf.cglib`/`org.springframework.cglib`, `javassist.util.proxy`) plus dynamic class loading (`Class.forName`/`defineClass`/`loadClass`/`new URLClassLoader`) — the latter outside the JVM-only `~/.forvum/plugins/` drop-in — and fails the build if any appear, excluding comment lines via a fixed-string allowlist (`native-discipline-allowlist.txt`, empty by design).
- **`@RegisterForReflection` audit.** A CI gate (`.github/reflection-registration.sh`, X1/#67) walks the DTO packages and fails the build if a record is missing the annotation. Records do not technically need reflection for their canonical constructor, but Jackson and Langchain4j both use it for field access, so the annotation is mandatory and enforced. The gate's scope is records in a `.dto.` package of a Quarkus-bearing module — the project's wire-DTO convention — which the reactor passes 100%. A *full* Maven-enforcer form (fail on any un-hinted serialized record, module-wide) is DEFERRED: a grep cannot distinguish a reflectively-bound record from one that only tree-walks a `JsonNode` (e.g. `QdrantConfig`/`ShellAllowlist`/`WebToolConfig`, which carry no annotation by design), so a module-wide rule would false-positive and force suppressions onto every config record; the scoped grep is the conservative, green-today gate. **This audit applies to DTO records in Quarkus-bearing modules (Layer 2+); `forvum-core` (Layer 0) is exempt.** Core bans `io.quarkus*` and cannot depend upward on `forvum-sdk`, so its records cannot carry the annotation — it lives in `io.quarkus.runtime.annotations`, and the SDK re-export (§2.2) sits *above* Layer 0, not below it. Layer-0 types that the engine serializes in native are instead registered from `forvum-engine` via a single `@RegisterForReflection(targets = { … })` holder enumerating the core records (compile-time-checked `.class` references, so a rename or removal breaks the build). That holder lands in the milestone that first serializes those types in native (M5/M6); M2 ships pure records with no annotation.
...
3. **Voice channel.** Local TTS/STT (Whisper + Piper) channel; delivered as `forvum-channel-voice`. *Realized (P2-3 #28):* a **file-drop** transport, NOT a live microphone — the inbound surface is a virtual-thread worker that polls `$FORVUM_HOME/channels/voice/inbox/` for audio clips and runs each through the pipeline (whisper.cpp STT → turn → piper TTS → reply WAV moved to the outbox), mirroring the Signal/Telegram poll-loop idiom (macOS `WatchService` latency, Risk #7, makes polling the right choice). The operator points `channels/voice.json` at their OPERATOR-installed whisper.cpp (`whisperBin` + `whisperModel`) and piper (`piperBin` + `piperVoice`) binaries; with any of the four absent the channel warns + no-ops (the CI no-config native-smoke contract), and `ChannelLauncher`'s serve-gate requires all four to MATCH `Spec.isReady()` (an enabled-but-not-ready config must not hang the binary in server mode serving nothing — the M17 trap). The external binaries run through a `ProcessBuilder`-based `SubprocessRunner` (concurrent stdout/stderr drains on virtual threads + a bounded post-kill drain so an escaped descendant holding a pipe cannot hang the worker). Live capture/playback (and barge-in) is a documented follow-up: it would pull `javax.sound.sampled` or a live-capture binary — a much larger native + cross-platform surface this v0.1 file-drop design deliberately avoids.
4. **Device pairing.** Pair a phone or second device to an existing Forvum instance, reusing identity and memory; delivered as `forvum-engine/pairing`. The engine submodule (P2-4) is "fixed code, configurable behavior": a device is paired by dropping a `$FORVUM_HOME/devices/<id>.json` (`identityId` + an optional `token` shared-secret + an optional `revoked` flag) — NO SQL table and NO Flyway migration (mirrors the `roles/`/`agents/` registries). A Layer-2 `Device` record (`@RegisterForReflection`), a `DeviceSpecReader` typed binder, and a `DeviceRegistry` (`@ApplicationScoped`, `ConcurrentMap` cache with IO off the lock + `@Observes ConfigurationChangedEvent` hot-reload) resolve a device endpoint id to its paired identity. Enforcement is at the turn entry `TurnService.dispatch`, keyed by `channelId` (the device endpoint), BEFORE the responder runs: an unknown or `revoked` device is rejected with `DeviceNotPairedException` (surfaced as the turn's terminal `ErrorEvent`). A paired device's `identityId` is RECORDED in its `devices/<id>.json` (the CLI/management surface in item 19 reads it); namespace sharing itself is delivered by the existing `IdentityResolver` channel-account mapping — `TurnService` resolves the session identity from `IdentityResolver`, so a device reuses an identity's memory namespace by mapping its `(channelId, nativeUserId)` to that identity, not via the `Device` record. Like RBAC, pairing is opt-in (an empty/absent `devices/` disables the guard — backward compatible, no migration), and the distinguished built-in `cron`/`server`/`cli` devices are always paired (exempt, mirroring item 11's `cron` role): the local operator CLI (`forvum ask`, channel `cli`) is the host's inherently-trusted primary surface, so enabling pairing never locks out the operator's own terminal. (`cron` turns never reach `TurnService.dispatch` — `CronScheduler.fire` calls `agent.respond` directly — so `cron` is exempt by construction; the `cron`/`server` `EXEMPT` entries are a defensive belt.) CLI surface (`forvum pair`/`forvum devices`), scope-upgrade approval, and `forvum doctor` drift detection are item 19 (P2-PAIR-SCOPE). **As-built (#166): device-token authentication + `approvedScopes` enforcement.** P2-4 parsed the `token` + `approvedScopes` but never compared the token nor intersected the scopes — a configured device was authorized by mere existence (the hardening audit's finding). #166 wires both, at the turn entry, BEFORE any model/tool runs: a transport-neutral `DeviceCredential(deviceId, token)` (a `forvum-core` record with a redacted `toString()`, so the secret never enters a log/error/ledger) rides a new `ChannelTurnDriver.dispatch` overload; `DeviceRegistry.authenticate(channelId, DeviceCredential)` does a constant-time (`MessageDigest.isEqual`) token compare + a `deviceId == channelId` channel bind (a credential issued for one channel/device cannot authorize another) + the revocation check, throwing `DeviceAuthenticationException` (a `DeviceNotPairedException` subtype, no token in the message); and `TurnService` intersects the device's `approvedScopes` into `CURRENT_EFFECTIVE_SCOPES` — but ONLY when the turn authenticated AS a device (a credential was presented), so a *pending* upgrade (requested-but-not-approved) never appears and the host operator (the ABSENT path) keeps its full scopes. The Web channel coordinates with the #165 `OperatorAuthMechanism` rather than adding a second credential mechanism: a token that is not the operator secret is matched (off the event loop) against each device's token and mints a `device`-role `SecurityIdentity`, which the `/ws/chat` policy admits alongside `operator` (the dashboards stay operator-only); `ChatSocket` reads that principal + the handshake token and propagates the `DeviceCredential`. **Token rotation/migration:** a device that declares a `token` now requires it presented; editing the file hot-reloads the new secret and the old one immediately stops working (single-token, no dual-accept window). A tokenless device file keeps the backward-compatible paired-by-existence behavior (the one-time migration is "declare a token to require it"). The local-transport exemption stays the explicit, in-code `cli`/`cron`/`server` carve-out — never inferred from an empty `devices/`. **As-built (#170): channel admission is fail-closed by default** — every remote channel denies all senders unless its `channels/<id>.json` lists `allowedUserIds` or sets `allowAllUsers` (a shared `ChannelAdmissionPolicy` in `forvum-sdk`, a startup audit naming each public channel, and `forvum doctor` validation). Device pairing itself stays opt-in (the deliberate #170 A1 scope decision); making device-token auth *mandatory* for remote channels is a future tightening.
...
10. **Provider onboarding wizard.** `forvum provider add <provider>` (P2-10) interactively onboards a key-based LLM provider: it prompts for the API key (no echo, via `Console.readPassword`), writes it owner-only (`0600`) to `state/credentials/<provider>` through the SDK `FileApiKeyStore` (the `CopilotCredentials` recipe generalized — secrets in a 0600 file, OpenClaw file-based parity, **NOT** a native keychain), runs a DB-free smoke chat against the provider to prove the key works, and offers to set `<provider>:<model>` as agent `main`'s `primaryModel` (editing `agents/main.json`, preserving unknown fields). The key-based providers (`anthropic`/`openai`/`google`) read this stored file in `resolve()` **only when** their `@ConfigProperty` api-key is blank, so an operator-exported `QUARKUS_LANGCHAIN4J_*_API_KEY` keeps precedence; the file read happens OUTSIDE the model cache's `computeIfAbsent` (no carrier pinning, [M7]). Ollama (keyless — set its base URL) and Copilot (`forvum copilot login`) are rejected with a pointer. Delivered as the SDK `FileApiKeyStore` + a `forvum-app` `ProviderCommand`/`ProviderAddCommand` — a `CommandMode` one-shot like `copilot login` (it only writes a credential file and runs a direct, ledger-free provider chat, so it skips the DB/watcher/cron boot and leaves HTTP unbound). The smoke resolves the raw `ModelProvider` (not the ledgering `LlmSelector`), so no `provider_calls` row is written. The default-chain update is single-link (`primaryModel`); `fallbackModels` composition is DR-4c/DR-8's, wired with the multi-link `LlmSelector` in P3 (#52). A native platform keychain (macOS Keychain, Secret Service, Windows Credential Manager) is a documented fast-follow.
...
12. **Structured output schemas per agent.** Agents declare an optional `outputSchema` (a JSON Schema) in `agents/<id>.json`; the `SupervisorGraph` parses the final assistant message as JSON and validates it against that schema. Delivered as an optional, backward-compatible `Persona.outputSchema` (a JSON-Schema STRING carried on the Layer-0 record — `null` = current free-text behavior, a present-but-blank value rejected; already in the §6.3 reflection holder), `AgentSpecReader` parsing it (an embedded object is re-serialized; a string taken verbatim), `GraphTurnRequest.outputSchema`, and a pure-Java `forvum-engine/.../graph/OutputSchemaValidator` that decodes the reply and validates it, re-serializing the validated `JsonNode` as canonical JSON on success. On failure (not valid JSON, missing required field, or a property of the wrong primitive type) the graph throws a `SupervisorGraphException` NAMING the schema and the offending field, which `TurnService` surfaces as a terminal `ErrorEvent` — no retry. **Decision (maintainer-locked):** JSON-Schema → `JsonNode`, NOT a typed POJO + LangChain4j `@Description`/`@StructuredPrompt` — a per-agent output class would force runtime reflection / classpath class loading and break the native binary; the string-schema + tree-walk path keeps it native-clean and config-driven (§5). A spawned worker child does not inherit the schema (its output is a digest merged as a tool result, never the validated top-level answer). **Fast-follow CLOSED (#124):** `OutputSchemaValidator` no longer hand-rolls the v0.5-parity subset (root `type`, `required`, primitive property `type`) — it now delegates to `com.networknt:json-schema-validator` (default dialect draft 2020-12), giving full draft coverage (`enum`, nested object/array schemas, numeric bounds, string length, `pattern`/`format`, `allOf`/`anyOf`/`oneOf`). The library proved GraalVM-native-clean: its bundled `META-INF/native-image/` metadata is an EMPTY `reflect-config.json` + a `resource-config.json` for its draft meta-schemas (zero runtime reflection, no `META-INF/services` ServiceLoader); the optional ECMA-262 regex engines (GraalJS/Joni) are `optional=true` and stay off the classpath. Proven by a deterministic, offline native IT (`OutputSchemaDoctorNativeIT`, default native leg — `forvum doctor` over a home whose agent declares a valid `outputSchema` exercises the library's factory init + bundled-meta-schema resource load + schema compile inside the native binary, no live LLM). `ConfigDoctor` now validates a declared `outputSchema`'s well-formedness through the SAME `OutputSchemaValidator` the graph enforces (an error becomes a doctor ERROR, not a turn-time surprise) — still reader-as-oracle, no parallel schema. The P2-9 config-loader half is otherwise UNCHANGED: it keeps the loader-as-oracle design (no formal schemas for `agents`/`crons`/`channels`/etc., no drift) — generating those FROM or testing them AGAINST the loaders is still a parallel definition that can drift, so the library is adopted only for the value-side `outputSchema` check where there is no loader to drift from.
...
14. **User-approval queue UI.** Dev UI and web-channel cards that show pending `USER_CONFIRM_REQUIRED` tool calls and let the user approve or reject them. Ratified expanded scope (§9.2.5): #39 (this wave) delivers the engine `USER_CONFIRM_REQUIRED` machinery + the SQLite-persisted blocking approval queue every shell/sandbox invocation parks through (timeout→deny), the web-channel approval card, and the dashboard route. *Realized (P2-14 #39):* `ToolSpec.userConfirmRequired` (additive 5th component, 4-arg ctor defaults `false`) opts a tool in; the engine `ToolExecutor` consults an `ApprovalService` (via the narrow `ApprovalGate`) as a THIRD gate, after belt + RBAC. A confirm-required call is parked in the SQLite `tool_approvals` queue (`V4__approvals.sql`) and the turn's virtual thread BLOCKS — resolved by a TTY `ApprovalPrompter` (interactive REPL), the web `/q/dashboard/approvals` decision endpoints (`POST .../{id}/approve|reject`), or a timeout / non-interactive context (`forvum ask`, cron, piped TUI) that denies. **Audit is two-surface (maintainer-ratified):** the queue row carries the lifecycle (`pending → approved|rejected|timed_out`); `tool_invocations` records exactly one terminal row (`ok|error|denied`), preserving the append-only ledger rather than parking a `confirm_required` row in it. The resolution mode is selected by per-turn `forvum-sdk` `ApprovalContext` bindings (`PROMPTER` / `NON_INTERACTIVE`), set at the turn entry. An approval orphaned by a process restart survives in the queue; approving it pre-authorizes the exact `(session,tool,args)` call (single-use) and re-dispatches the turn from its stored user message (**R1**, best-effort/non-deterministic; exact checkpoint/resume is **R2**, tracked as #138). The Dev UI card is a descoped fast-follow (dev-mode only, non-native); the native web surface (the JSON dashboard `@Route` + decision endpoints, mirroring `CaprDashboardRoute`) is what v0.5 ships.
...
18. **QA scenario suite.** `forvum qa suite` / `forvum qa <channel>` runs a scripted scenario pack against the running assistant; fails by default on any unverified scenario. The scenario pack ships in the release and the suite is a CI gate. *Realized (P2-QA):* `forvum-app` `QaCommand` (registered in `RootCommand`; NOT a `CommandMode` one-shot — it runs turns, so it needs Flyway/Panache) loads the bundled `qa/scenarios.json` classpath resource (override with `--pack <file>`; hinted into the native image via `quarkus.native.resources.includes=qa/**`) and runs each scenario through the real turn path (the SDK `ChannelTurnDriver`/`TurnService`, like `forvum ask`). **Scenario format** (the ONE shared shape with the eval harness #58 — `docs/SCENARIO-FORMAT.md`): a pack is `{ "scenarios": [ { "id", "channel", "prompt", "expect", "match": "contains|exact|regex" } ] }`, where the per-scenario `id`/`prompt`/`expect`/`match` case fields are identical to eval's and scored by the SAME engine `MatchMode` matcher; `channel` is qa's own suite-level concern, and a blank/absent `match` defaults to `contains`. CI needs NO live inference: the seeded home pins `main` to a bundled, network-free `echo` `ModelProvider` (extension id `echo`, INERT unless pinned — like the bundled Qdrant memory provider) whose reply is `"echo: <prompt>"`, so an `expect` is deterministic on the JVM jar AND the native binary. Fails-by-default: an empty/absent pack or a channel target matching no scenario exits 1 (never a vacuous pass); any failed scenario exits 1; logs go to stderr, the summary to stdout. The CI gate is a separate `.github/workflows/qa.yml` (JVM fast-jar + native binary), plus the in-suite `QaSuiteNativeIT` running in `ci.yml`'s default native leg.
...
22. **Cron isolated-agent delivery modes.** `delivery.mode: none | last | explicit-to` on cron entries, with per-execution dedupe and ambiguous delivery rejected at add/update time. Folds into the item 11 RBAC `cron` role. *As-built (P2-CRON-DELIVERY):* the `delivery` block parses to a typed `Delivery` whose canonical constructor rejects the mode↔target ambiguity, and `CronSpecReader` rejects an `explicit-to` target that is not a configured channel (the `channels/<id>.json` set) — both at PARSE, so `CronScheduler` disables the bad cron and `forvum doctor` surfaces it. Routing is inline in `CronScheduler.fire()` after a successful turn, once per fire (in-execution dedupe; no table/migration), through a `CronDeliverySink` seam. The outbound surface shipped with #188: the `forvum-sdk` `ChannelSender` SPI backs the sink, so `explicit-to` reaches the named channel's installed sender and `last` reaches the most recent sender-capable session's native user — hardened post-audit so `last` delivers ONLY to a destination present in the channel's configured `allowedUserIds` (the sessions ledger is history, not authorization) and every cron reply passes the `OutputGuardChain` (`PRE_CHANNEL_EMIT`) before any sender sees it (Blocked → suppressed entirely, reason logged, payload never). Telegram is the sole `ChannelSender` in-tree; other outbound-capable channels are deferred follow-ups. Model-callable sends are the separate allowlist-gated `message.send` envelope (`tools/message-send.json`, fail-closed, `userConfirmRequired`, the engine-side `MessageAccess` seam). When no sender matches (or membership/config refuses), delivery degrades to the logged sink — the pre-#188 cron contract preserved.
...
24. **Text-to-speech tool.** A model-callable `tts.speak(text, [voice])` tool that synthesizes a WAV under the workspace by driving the operator-installed piper binary (OpenClaw parity — OpenClaw exposes TTS as a tool, whereas v0.5 Forvum had piper only inside the voice *channel*). *Realized (#186):* delivered as `forvum-tools-tts`, the shell/filesystem Layer-3 tool recipe (`forvum-sdk` + `quarkus-arc` + `quarkus-jackson`, no `build` goal, copied enforcer allowlist). A NEW `PermissionScope.MEDIA_SYNTHESIZE` gates it (auto-flows into the permissive `default-user` role; a role can withhold audio synthesis without withholding filesystem write). The operator points `tools/tts.json` at their piper binary (`piperBin`), a default ONNX voice (`piperVoice`), and an optional `voices` NAME → path map (so the `[voice]` parameter selects a voice by name — the model can never point piper at an arbitrary path); the config is read on demand into a hand-parsed record (no `@RegisterForReflection`), inert (an actionable "not configured" error, never a crash/hang) when absent. Output is a generated collision-free name (`speech-<yyyyMMdd-HHmmss>-<8 hex>.wav`) under a fixed `<workspace>/tts/` subdir; there is no model-supplied path to confine. `userConfirmRequired=false` (fs-write-class: operator-fixed program + argv + voice + output location, model controls only the stdin text and a config-resolved voice name — unlike `shell.exec`, which confirms because the model chooses the program). The piper subprocess is driven through the module's own copy of the voice channel's `ProcessBuilder`-based bounded-drain `SubprocessRunner` (stdin write-then-close since piper is stdin-fed; `{PATH, HOME, LANG}` env scrub; kill-tree + bounded post-settle drain), so the JVM links no audio codec. A voice-provider abstraction (piper is THE backend) is `GenerationProvider` (#187) territory. The real piper round-trip is a `@Tag("live")` test, default-off (nightly/manual), never gating the native compile — the shell/voice precedent.
25. **Image + PDF analysis tools.** Model-callable `image.analyze([paths], [prompt])` and `pdf.analyze(path, [prompt])` tools that read a workspace image or PDF and route it through a vision-capable sub-generation for a text analysis — Forvum's input path was text-only. *Realized (#185):* delivered as `forvum-tools-multimodal`, the Layer-3 tool recipe (`forvum-sdk` + `quarkus-arc` + `quarkus-jackson` + Apache PDFBox, no `build` goal, copied enforcer allowlist, **no langchain4j dependency**). A NEW `PermissionScope.MEDIA_ANALYZE` gates both tools (auto-flows into the permissive `default-user` role; the restricted `cron` role excludes it). The vision routing lives behind a Resolution-B `MediaAnalysis` seam in `forvum-sdk` (the `ChannelTurnDriver`/`MemoryAccess` idiom — engine sole implementor `EngineMediaAnalysis`): it resolves the model (an explicit `tools/multimodal.json` `model` override, else the agent's primary — lease-aware via `CURRENT_AGENT_SPEC`, #178) and drives it through the budget-gated (#169), `provider_calls`-ledgered, OTel-spanned `LlmSelector.resolve(..., costBudget)` path, so a sub-generation consumes the cost budget like any other call. Images go to every installed provider as `ImageContent`; `pdf.analyze` is dual-path — native `PdfFileContent` where the provider maps it (anthropic/google/openai), else local **text-layer** extraction via PDFBox `PDFTextStripper` (no OCR, no rendering) injected into the prompt inside a delimited, closing-tag-neutralized data block (untrusted-text framing). `userConfirmRequired=false` (read-only over the workspace + a model spend — the `web.fetch` posture); the tools are functional with no config, so there is no `configGaps` entry. PDFBox is adopted via the [#124] narrow-adoption protocol: no bundled native metadata / no ServiceLoader / BC optional-and-excluded (encrypted PDFs rejected), the only native work a hand-authored `resource-config.json` embedding the standard-14 AFM/glyphlist/cmap resources; `commons-logging` is bridged to jboss-logging for native-cleanliness. The workspace-media read path copies the filesystem module's hardened `WorkspaceRoot` (lexical + `toRealPath` NOFOLLOW confinement). The live native vision/PDF turn is proven by a local `-Pnative` build (the [M14]/[#186] Layer-3 native discipline), not a per-PR CI job.
26. **Structured planning tool.** A model-callable `update_plan(steps[])` built-in that records a structured plan + per-step status into the session scratchpad, so multi-step turns keep a durable, window-independent checklist (OpenClaw parity; the **Write** pillar's strongest multi-step-coherence driver). *Realized (#190):* an engine-resident built-in offered alongside the belt exactly like `spawn_worker` — hand-built `PLAN_SPEC`, **no `PermissionScope`**, always available, intercepted at the top of the graph's `runTool` (after the replay short-circuit, so replay stays write-free), zero `ToolCallBridge`/SDK/app changes. Each call is full-plan-replace, validated engine-side (`PlanFormat`: required `plan[]` of `{step, status: pending|in_progress|completed}`, optional `explanation`, ≥1 step, ≤1 `in_progress`, caps 50 steps / 300 chars-per-step / 8 000 rendered chars — over-cap rejects with a model-visible error, never truncates, nothing written) and persisted append-only as ONE `messages` row (`role='tool'`, new code-only `BlockType.PLAN("plan")` — V3 has no CHECK constraint, no migration) whose content is the RENDERED checklist text (`[ ]`/`[>]`/`[x]`), so there is zero new reflection surface. The live plan (newest row per session+agent) is injected ONCE at turn entry after memory retrieval, framed as a closing-tag-neutralized `<current_plan>` DATA block positioned directly before the user's question; same-turn visibility rides the tool-result echo. Compaction (P2-COMPACT) gains an explicit `PLAN` case: the region's newest plan row is retained verbatim regardless of age, superseded plan rows are stripped as orphans, and plans are never summarized. `GraphState` stays serialization-clean (R6) — plan content lives in SQLite, never in graph channels.
27. **Media-generation tools.** Model-callable `image.generate(prompt)` / `video.generate(prompt)` / `music.generate(prompt)` tools that turn a prompt into a media asset (OpenClaw parity — OpenClaw ships `image_generate`/`video_generate`/`music_generate` with pluggable backends; Forvum had no generative-media surface). *Realized (#187):* a NEW sealed `GenerationProvider` SPI in `forvum-sdk` (with the `non-sealed AbstractGenerationProvider` leaf — the P2-5 `MemoryProvider` recipe, NOT a `ModelProvider` overload: a prompt→bytes generation does not fit the LangChain4j `ChatModel`/`EmbeddingModel` shape and must span video/music), carrying pure SDK values `GenerationRequest`/`GeneratedMedia`/`MediaKind`; `generate` is BLOCKING on the caller's virtual thread. Delivered as the Layer-3 `forvum-tools-media-gen` module (the qdrant/telegram blocking-REST recipe over `quarkus-rest-client-jackson`): `MediaGenToolProvider` contributes the three tools (all gated by the NEW `PermissionScope.MEDIA_GENERATE` — one scope, since all three share the prompt-to-asset shape and their only external effect is a backend spend + a workspace write; auto-flows into `default-user`; `userConfirmRequired=false`, the `tts.speak` posture — the model controls only the prompt) and routes the requested `MediaKind` to an installed `GenerationProvider` discovered via CDI `Instance` (active preferred, inactive fallback so ITS "not configured" error reaches the model, no candidate = an actionable capability-gap error). The bundled backend is `OpenAiImageGenerationProvider` (images-only): `POST /v1/images/generations` with `response_format=b64_json` via a per-invocation `@Url` override, configured by `tools/media-gen.json` (`baseUrl` + optional `apiKey`/`model`, hand-parsed JsonNode tree-walk — inert with an actionable error when absent; the #184 `configGaps` names the file+field). Outputs land as generated collision-free names under a fixed `<workspace>/media/` outbox (no model-supplied path to confine, atomic-move write). REST DTOs carry `@RegisterForReflection`; a `@Tag("live")` real generation (nightly `media-gen` job, OpenAI-secret-gated) covers the wire path; adding a video/music backend is a Java plugin implementing the same SPI (the documented native trade-off, §6.2/§6.3).
...
4. **CAPR-driven adaptive model routing.** The `LlmSelector` consults rolling CAPR per-model over the last N turns and down-ranks models with sagging pass rates automatically. The router is itself a small local model that sees the CAPR snapshot at every decision. *Implemented as the DETERMINISTIC blended router (#52, DR-4c [DP-8]): `LlmSelector.select` composes the persona's `FallbackChain.links()` (primary + `fallbackModels`) and `CaprRouter` reorders them by a recency-weighted blended score so a sagging model sinks below a healthier sibling — never inventing or dropping a declared model, declared order as the deterministic tiebreak, cold-start/tie stable (neutral prior). The per-model health signal is the `provider_calls` ledger (each row carries `provider`/`model`/`error` — the genuinely per-model pass/fail; `capr_events` carries no model column and only placeholder `passed=1` verdicts in v0.1) read over a rolling window by `ModelHealthReader`. Knobs (MP Config, neutral defaults): `forvum.routing.capr.{enabled=true, weight=0.7, window=20, min-attempts=3}`; no LLM call on the hot routing path (the "router is a small local model" framing stays the v1.0 aspiration) and no schema change. The single-ref `resolve(ModelRef,...)` path (cron/replay/compaction/worker) is always single-link, so routing is a no-op there. The real per-turn judge that would replace the placeholder `passed=1` verdict — closing the CAPR success-metrics loop (cf. Risk #10) — has since shipped as **#195** (see the §7.3 post-v0.5 addendum, item 11).*
5. **Multi-user toggle.** The same binary serves both modes. *Implemented (#53): per-IDENTITY rows in ONE SQLite (not per-user `$FORVUM_HOME` dirs or separate schemas — the ratified simplification). `forvum.multi-user.enabled` (default false) is the toggle: off binds the `"default"` tenant identity for every turn (byte-identical to single-user), on binds the resolved identity. `semantic_memory` gains an `identity_id` column (V5, `UNIQUE(identity_id, agent_id, key)`, `DEFAULT 'default'` so existing facts migrate losslessly) — the only state not already identity-scoped via the `sessions.identity_id` FK. `CurrentIdentity.CURRENT_IDENTITY_ID` is the ScopedValue tenant key (bound at `TurnService.dispatch`/`CronScheduler.fire`, the P2-11 pattern); `AgentMemory.recordFact` writes per-identity and `factValue` reads the owning identity's fact, falling through to the shared `"default"` (team-skill) namespace. Cross-identity vector retrieval (Qdrant metadata filter) and a `tasks` identity column are deferred. **As-built (#175, done):** the three-tier Write/Select loop is now wired into normal turns — after each turn `MemoryWriter` extracts durable facts with the small model, runs them through the pre-memory-write secret filter, embeds them, and upserts `semantic_memory` (off-turn), and `recordTurn` writes the real turn content to `episodic_memory`; the bundled local `LocalMemoryProvider` reads both tiers back on the next turn (identity/agent-scoped), with `MemorySelector` preferring a configured external provider (Qdrant) only when active. Cross-identity vector retrieval (Qdrant metadata filter) and a `tasks` identity column remain deferred.*
...
9. **Queryable session replay via SQL.** Every session is replayable with substitution because the schema captures everything — not possible in OpenClaw's JSONL store without custom parsing. *Implemented (#57): `forvum replay <session> --model <ref>` re-runs the stored user turns under a substituted model into a NEW session (never in-place), reusing the recorded tool outputs FIFO-per-tool (a miss → a synthetic "recorded output unavailable" marker), and records the substitution in the new session's `metadata_json` (zero schema change). A per-replay `ReplayContext.CURRENT_REPLAY` ScopedValue puts the graph in replay mode — recorded tool outputs replace live execution, and memory retrieval + proxy compression are disabled — so the rerun is deterministic against the same tool outputs the original turn saw. Model-substitution + tool-output-substitution only; memory-policy substitution is descoped.*
...
**Post-v0.5 context-engineering differentiators (filed 2026-06-27).** Beyond the original ten, a post-v0.5 OpenClaw-parity + Context-Engineering audit surfaced three further v1.0+ bets, each sharpening one pillar to where `CONTEXT-ENGINEERING.md` currently out-aims the as-built runtime. All three have since shipped (see the *Realized* notes below); not part of the original P3-1…P3-10.

11. **Real per-turn CAPR judge (#195).** Replace the placeholder `passed=1` verdict (item 4) with a genuine LLM-as-judge — reusing the offline eval machinery (`LlmJudge`/`MatcherJudge`), off the critical path and off by default — plus the `capr_events.model` column `ModelHealthReader` anticipates, so routing reflects answer quality, not just call health. Closes the success-metrics loop. *(CAPR pillar; cf. Risk #10.)* *Realized (#195): `TurnJudge` hooks `Agent.onTurnCompleted()` and — when `forvum.capr.judge.enabled=true` (default false) — judges the turn asynchronously on a virtual thread using the configured judge model (`forvum.capr.judge.model`, default the small-and-fast `ollama:qwen3:1.7b`); the NaN-guarded `TurnVerdict` carries pass/fail + a [0,1] score, and `CaprRecorder` writes real `capr_events` verdicts (answering model attributed via the `provider_calls` ledger, judge model recorded) with a graceful judge-unavailable fallback; the disabled path keeps the neutral `recordPassed` row so routing history stays comparable.*
12. **Iterative / agentic RAG (#196, blocked on #175).** Upgrade Select from single-shot `<retrieved_memory>` framing to a bounded retrieve → evaluate → decompose loop, optionally as an Isolate-respecting memory sub-agent (only a compressed digest crosses `reduce`) — the `CONTEXT-ENGINEERING.md` "Agentic RAG dinâmico" ask. *(Select pillar.)* *Realized (#196): a new `RetrievalStrategy.ITERATIVE` on `MemoryPolicy` (OPT-IN; default stays single-shot) swaps `SupervisorGraph.retrieveAndFrame`'s turn-entry retrieve for `IterativeRetrieval` — a BOUNDED retrieve → evaluate-sufficiency → decompose/re-query loop (≤3 iterations, ≤3 sub-queries each) run as a blocking memory sub-agent on the turn's model with a plain-text SUFFICIENT/INSUFFICIENT verdict protocol (no JSON, reflection-free, native-clean); the evaluator transcript never enters the supervisor window (Isolate: only the accumulated hits cross back, then the normal compress + `<retrieved_memory>` DATA framing), each provider call runs the HYBRID blend, and evaluator failure degrades gracefully to what was already retrieved (only `BudgetExhaustedException` re-throws).*
13. **Mid-turn context pruning (#197).** Add within-turn pruning of oversized tool results and out-of-window images (tail-first, prompt-cache-stable), complementing the between-turn `SessionCompactor` and the item-8 proxy-compression middleware. *(Compress pillar.)* *Realized (#197): `MidTurnPruner` (static, zero new config) runs before every model call inside `SupervisorGraph.generate()`, operating in-place on the turn's mutable conversation past the frozen-prefix boundary (never insert/remove/reorder — prompt-cache-stable). Two arms (post-audit): oversized tool results are middle-elided (head+tail kept under the threshold, idempotent) — but only OUTSIDE a `KEEP_LAST_ASSISTANTS = 3` recency window (D3.3, the last three rounds' results always reach the model whole) and never for a `PROTECTED_TOOL_NAMES` tool (`update_plan`, D3.4, the checklist IS the same-turn plan surface) — and images outside the image recency window are replaced with a marker (as-built: this arm is DORMANT — #185 confines `ImageContent` to the isolated media sub-generation, so images never enter the supervisor list today). The originally shipped third arm (stripping superseded `AiMessage.thinking()`) was REMOVED by the #197 audit: it rewrote already-sent assistant messages, breaking every prompt-cache prefix and rejected outright by providers requiring thinking blocks paired with their `tool_use` (Anthropic extended thinking); D7 keeps thinking-stripping out of v1. Governed by the EXISTING `MemoryPolicy.compressThresholdChars` knob (≤0 disables the whole pass, preserving replay determinism); pure in-memory, no schema impact; the reflection-cycle path (`runCycle`) intentionally runs no pruning — cycles execute no tools, so nothing oversized accumulates there.*
...
- **D (Denial of service) — destructive-action storm.** Destructive tools (e.g. `shell.exec`) are intended to sit behind a `USER_CONFIRM_REQUIRED` approval hook (§2.4, §5.5); the call would be parked as `confirm_required` until the owner approves (P2-14 #39 supplies the per-channel approval UX). This is now **built**: the #39 approval machinery landed (PR #137 — `USER_CONFIRM_REQUIRED` in `ToolSpec`, the `ToolExecutor` third gate, the SQLite-persisted blocking queue with timeout→deny), and `forvum-tools-shell` (#27, PR-6) ships `shell.exec` behind the `tools/shell.json` allowlist with `userConfirmRequired=true` — its first true consumer — so every invocation parks through the #39 gate. The belt gate and the RBAC role gate above are the additional destructive-action controls (M13 / P2-11). (The podman sandbox sibling reusing `SHELL_EXEC` is the deferred #27 fast-follow.)
...
- **Mitigation [contract].** A single **outbound filter** runs at the **pre-channel-emit** hook layer on every channel egress surface — the `OutputGuard` SPI (§7.2 item 23) implementing the `OutputFilter` contract specified in §9.2. Because every channel emits through the same `AgentEvent` → channel-render seam (§5.3, outbound flows as `AgentEvent.TokenDelta`), one filter placement covers all three v0.1 channels and every future channel. The filter can **block**, **redact**, or **mark-filtered** the egress (§9.2 `FilteringOutcome`). **[contract — §9.2; built by P2-OUTPUTGUARD #48.]** **As-built (#172): the ERROR egress is now sanitized too.** P2-OUTPUTGUARD wired the SUCCESS reply through `OutputGuardChain` but the five `ErrorEvent` construction sites (all in `TurnService.dispatchAuthenticated`, rendered `.message()`-verbatim by every channel/CLI/Voice-TTS surface) bypassed it — a provider/tool/network exception could leak secrets, private paths, tool arguments, or prompt fragments (the `codex-review` MEDIUM finding). A single `emitError` seam now sanitizes every error at construction: the untrusted `turn_failed` message is genericized to a stable category + the root-cause simple class name + the curated connection hint + a `turnId` correlation `ref` (never the raw `getMessage()`/cause text — `SecretRedactor` cannot guarantee no non-secret-shaped path/arg/prompt leaks, so the raw text is dropped, not merely redacted); curated config-error messages pass through the `SecretRedactor`; the full detail is logged (redacted) at WARN keyed by `turnId` (the protected operator diagnostic); and the channel-bound event keeps `exceptionClass` (a class name — a code identifier, safe, kept for telemetry + type assertions, never rendered by a channel) but nulls `stackTraceText` (the full stack carries exception messages — the latent serialization leak — kept only in the redacted internal log). **[built — #172.]**
...
#### 9.1.f Channel transport — device/connection spoofing (#166, built)

- **Threat (S — Spoofing).** A connection reaches or impersonates a channel without presenting the paired device's secret, then inherits scopes the operator never approved for that device. P2-4 made this real: pairing was keyed only on a `devices/<id>.json` *existing* — the `token` was parsed but never compared and `approvedScopes` was never intersected, so one declaration authorized every connection on that channel.
- **Mitigation [built — #166].** The device credential now participates in the turn authorization decision. A transport-neutral `DeviceCredential(deviceId, token)` is carried from the channel adapter into the turn; `DeviceRegistry.authenticate` does a **constant-time** (`MessageDigest.isEqual`) token compare + a `deviceId == channelId` bind + the revocation check, rejecting a missing/invalid/cross-channel/revoked credential with `DeviceAuthenticationException` **before** any model/provider/tool call; and the device's `approvedScopes` are intersected into `CURRENT_EFFECTIVE_SCOPES` (§9.1.b) when the turn authenticated as a device, so a pending upgrade never grants a scope. The secret never enters a log/error/ledger (`DeviceCredential.toString()` redacts it; no token in any exception message). The Web channel reuses the §9 / #165 authenticated-principal seam (`OperatorAuthMechanism` mints a `device`-role identity for a device token, admitted on `/ws/chat`), not a second credential mechanism. The local-transport exemption (`cli`/`cron`/`server`) is the explicit, in-code carve-out, never inferred from an absent `devices/`. **[built — #166.]** **As-built (#170): channel admission is fail-closed** — a remote channel admits no sender without an explicit `allowedUserIds` or `allowAllUsers`, closing the audit's permissive-default path (the integration layer composing #166/#167/#168). Device pairing stays opt-in (the #170 A1 scope decision), so making device-token auth itself mandatory for remote channels remains a future tightening.
...
- **Every invocation is `USER_CONFIRM_REQUIRED` — allowlisted or not.** The allowlist bounds *what can run*; the confirm gate guarantees *the owner sees each run* (defense in depth on top of the §9.1.b belt + RBAC gates, which also still apply). Each call is parked through the **P2-14 #39 approval queue — blocking, SQLite-backed**: the tool call's virtual thread blocks on the queued row until approve/reject; an unresolved confirmation times out to deny (the bound is `forvum.approval.timeout-seconds`, default 300). Audit is **two-surface** (P2-14, maintainer-ratified): the `tool_approvals` queue (`V4__approvals.sql`) carries the approval lifecycle (`pending → approved|rejected|timed_out`), while `tool_invocations` records exactly one terminal row (`ok`/`error` on an approved run, `denied` on the engine's own reject/timeout verdict) — preserving the append-only ledger rather than parking a `confirm_required` row in it (the original `confirm_required`-in-`tool_invocations` plan was superseded; the queue owns the parked state). A **shell-tool-internal refusal** (empty/missed allowlist, invalid argv, or workspace escape) surfaces as a thrown exception the engine's generic post-action catch audits **`error`**, not `denied`: `DENIED` is the engine's *pre-action* verdict from its own belt/RBAC/approval gates (which know the engine exception types), which a Layer-3 tool cannot produce — the refusal is still non-repudiably logged. This matches the ratified WorkspaceRoot-escape `error` audit (#27). **[DP-9: confirm-required on every shell/sandbox invocation via the #39 blocking SQLite queue. Ratified (wave directive 2026-06-09); audit refined to the two-surface model (2026-06-17); allowlist-miss/escape audited `error` since a Layer-3 tool cannot emit engine-DENIED, landed with #27 (2026-06-17).]**
...
A skill is **operator-trusted CONTENT, never code** (unlike OpenClaw, whose skills can carry executable code and are therefore "untrusted code" — Forvum's surface is prompt-template-only). `skills/<skill>.md` carries **full front-matter input-schema** declaration ([ratified] for #32); the schema is validated on read (file-naming error on a malformed skill, the M4 reader convention) and invocation arguments are validated against it before expansion. The expanded template is content inside the invoking agent's turn: any tool call it induces executes under that agent's **existing** belt and the caller's effective scopes — a skill carries no scope set, no belt, no identity, so **escalation-by-skill has no mechanism**; a malicious skill is exactly the §9.1.c injection threat, contained by the same structure. #32's installer writes the file owner-only (`0600`, the `InitCommand` recipe); `skills/` is hot-loaded (`ConfigWatcher.WATCHED_SUBFOLDERS`). **[6b-DP-7, ratified]** The `SkillToolProvider` declares a `requiredScope`; **[6b-DP-8, landed #191]** the dedicated `SKILL_INVOKE` constant (read-only class, `userConfirmRequired=false`) gates both `skill.invoke` and `skill.list`. It auto-flows into the permissive `default-user` role (`EnumSet.allOf`); `cron` (`{FS_READ}`) and `anonymous` (`{}`) are withheld — an operator grants it to a restricted role via `roles/<name>.json`.

#### 9.3.4 Maven-plugin supply chain (#31 as-built; build-input supply chain stays DR-6c §9.4)

`MavenPluginResolver` resolves `groupId:artifactId:version` against `~/.m2/repository` then Maven Central over **HTTPS** (`https://repo.maven.apache.org/maven2/`; the `forvum.plugins.repository-url` override exists for local mirrors + hermetic `file://` tests). Maven Resolver fetches and checks SHA-1 checksums for every download, but the pre-#171 default `RepositoryPolicy` was checksum-**`warn`** (verified against `maven-resolver-api` 1.9.x bytecode) — a silent integrity bypass on the one path that pulls executable code from the network. **As-built (#171):** the resolver enforces `CHECKSUM_POLICY_FAIL` explicitly on BOTH the session (`session.setChecksumPolicy`, a global override) AND every remote repository (an explicit `RepositoryPolicy` on release and snapshot in `remote()`), so a **missing OR mismatched** checksum aborts resolution and leaves no loadable JAR — independent of Resolver defaults; the local `~/.m2` cache resolves without re-verification (the operator's own disk, inside the install-act-is-the-trust-decision boundary — identical to Maven itself). The repository URL is restricted to a `{https, file}` scheme allowlist — plaintext `http://` (a MITM downgrade) is rejected; `file://` is retained for local mirrors and the hermetic test path (checksum FAIL still applies to it). Documented residual: an approved https repository that redirects to http is followed by Resolver 1.9.x's HTTP transport — accepted because repositories are operator-approved, and disabling redirects would break legitimate https-to-https mirrors. Resolution-failure diagnostics redact `://user:secret@` URL credentials. **PGP signature verification is a documented deferral** (key-trust management is a real subsystem; Maven itself does not verify by default) — re-opens with DR-6c's build-input work if at all. The installer now creates `plugins/` and the installed JAR **owner-only** (`0700`/`0600` on POSIX, the `InitCommand` recipe, carried in a package-private `PluginArtifactInstaller`) via an atomic temp-file + `ATOMIC_MOVE` (never a partially copied JAR; a failed install never alters the previously installed valid plugin), rejecting a symlinked `plugins/`/target and **failing** the install (fail-closed, exit 1 with remediation) if a pre-existing loose dir cannot be tightened — deliberately not the boot path's repair-and-warn, since an interactive one-shot installer staging executable code is not the M4 graceful-boot contract. `plugins/` is deliberately not read at boot (only `PluginInstallCommand` consumes it — drop-in loading is an operator classpath action after restart), so install-time is the enforced gate; a future boot-time drop-in scanner must re-validate then. On a native binary the command stages-and-warns (`ImageMode.NATIVE_RUN`) — correct as-built: the drop-in path is JVM-fast-jar-only by design, and the fixed native code set is itself a supply-chain control. **[6b-DP-10]**
...
- **Supply chain.** The **native binary is locked at build time** `[6c-DP-9]`: plugins load only from the compile classpath (the `~/.forvum/plugins/` drop-in is JVM-fast-jar-only by design, §6.2), per-module `maven-enforcer-plugin` `bannedDependencies` allowlists police the layer graph (live since M1), `forvum-bom` is the single version bump point excluding the vetoed dependencies (§6.3), CI pins the exact Mandrel patch, and the §6.3 banned-import CI greps complete the enforcement with X1 (#67). **CI security gates (#174)** add the boring, widely-adopted repository/build/release controls: a per-PR blocking dependency-diff (`dependency-review`) + secret scan (`gitleaks`, with the `**/src/test/**` fake-fixture allowlist) + `CodeQL` (Java + the Actions workflows) + `actionlint`/`shellcheck`; Dependabot alerts/security-updates as the continuous Maven-vulnerability signal + a weekly Trivy deep scan of the shipped image; and every third-party Action pinned to an immutable commit SHA (GitHub-authored actions stay on major tags), with Dependabot as the update process. **JVM drop-in plugins are an operator-trust boundary** — the contract is §9.3 (DR-6b), cross-referenced, not duplicated `[6c-DP-10]`. **Release artifacts ship with `SHA256SUMS`** published alongside them (#49; the installer verifies its download against it before installing). **SBOM + signed provenance are shipped as of #174** `[6c-DP-11]`: every release carries a CycloneDX SBOM for the Maven closure **and** for the container image, plus GitHub OIDC build-provenance attestations for the four platform binaries and the GHCR image (verification commands + the committed threshold/suppression/SLA policy live in `docs/SECURITY-GATES.md`); the container image is scanned (blocking, pre-push) before it reaches GHCR. SBOM *attestation* (`actions/attest-sbom`) is the one named follow-up recorded in that doc.

- **Secrets at rest.** The posture is the **owner-only `$FORVUM_HOME` tree** — `forvum init` creates `0700` dirs / `0600` files (POSIX-guarded) — and the #35/#42 **credentials JSON joins it at `0600`** as the v0.5 storage surface for provider keys and OAuth tokens; the §4.1 platform-keychain direction (macOS Keychain / Secret Service / Windows Credential Manager) remains the named hardening follow-up, not the v0.5 gate `[6c-DP-12]`. Channel `botToken`s stay file-borne as built (`channels/telegram.json`, `channels/discord.json`). `StateDirInitializer` enforces the same owner-only recipe on **every** boot (#173): on POSIX it creates `state/` `0700` independent of umask, repairs a pre-existing world-accessible `state/`, and tightens the SQLite database + WAL/SHM sidecars to `0600` after migration — best-effort (repair-and-warn, never blocking the boot; a symlinked state path is rejected), so a no-`init` first boot no longer leaves the store group/world-readable `[6c-DP-13]`. **No-secret-in-logs is a standing review obligation** `[6c-DP-14]`: every secret-bearing surface ships its redaction seam (the Telegram `/bot<TOKEN>` and Discord `Bot <token>` precedents) plus a non-live log/encode test in the same PR.
...
- **Coverage policy.** §3.7's JaCoCo 80 % line gate plus a 75 % branch gate. *Operationalized (X3):* the `jacoco-maven-plugin` `check` (a BUNDLE rule, 80 % LINE / 75 % BRANCH) is declared once in the parent and **inherited per module**, so it gates each module's own coverage rather than a reactor-aggregate number a weak module could hide inside; it runs in `verify` over the Surefire unit run only (the native-profile Failsafe `*IT` smoke is excluded from coverage). Baselines were measured before the gate landed; #180 then REMOVED the substantive exclusions: only structural entries remain (`forvum-sdk`'s logic-free sealed-set `Abstract*Provider` bridges; `forvum-engine`'s native-metadata holders + pure Panache `*Entity` data classes — the formerly-excluded Panache-calling services now sit in the denominator, offset by direct-instantiation unit tests), `forvum-channel-telegram` is back on the inherited global gate (a lifecycle unit test drives the CDI observer paths), and the two remaining per-module threshold exceptions (`forvum-app` BRANCH 0.70, `forvum-channel-discord` LINE 0.75, both IT-only boot/error paths) are ratified, owner-tagged, expiry-bound entries in `.github/coverage-allowlist.txt`. The `.github/coverage-policy.sh` CI gate fails any jacoco exclusion or lowered literal minimum that is not in that allowlist (and prunes stale entries), so the gate cannot silently erode. Mutation testing (Pitest) landed in `forvum-core` first because its Quarkus-free domain types are mutation-friendly (#182): the measured Java-25 baseline is **97 % mutation-killed / 97 % test strength** (175 mutants, 0 no-coverage), so the ratchet is wired as a **failing gate** — `-Pmutation` enforces `mutationThreshold=95` / `testStrengthThreshold=95`, coverage-policy.sh direction-guards the floor (it may only go up), and `.github/workflows/mutation.yml` runs it per-PR-on-core-change + weekly with a survivor summary. (`forvum-sdk` is also Quarkus-free but is dominated by sealed interfaces and abstract classes — little behavior to mutate; mutation joins it in a later phase alongside `forvum-engine`.) Coverage gates are gates; the core mutation ratchet is now also a gate.
...
- **Performance gates per turn — ENFORCED (X4/#70).** p95 first-token latency excluding model inference: TUI ≤ 200 ms, Web ≤ 300 ms, Telegram ≤ 500 ms. Enforced by `ChannelLatencyGateTest` (a `forvum-app` `@QuarkusTest` run under `./mvnw verify`): it drives the real turn through the shared SDK `ChannelTurnDriver` (the engine `TurnService` every channel calls) with the in-process `FakeModelProvider` (deterministic `"pong"`, so the gate measures Forvum, not the LLM), warms persistence in `@BeforeEach`, and over 60 measured dispatches per channel asserts the **median ≤ the budget** plus a **p95 ≤ budget × 3 CI-headroom ceiling**. The amendment (with evidence, per the §5 carve-out) is intentional: a warm fake-model turn pays a real per-turn SQLite round-trip (ensure-session + the eager session-compaction read), so the measured median first-token is ~80–110 ms and a loaded CI cell's GC/scheduler outliers blow the raw 200 ms p95 even though the typical turn is well inside it (observed `median=88 ms, p95=317 ms` on a heavily-contended cell). The median enforces the documented budget (the typical-turn regression signal); the ×3 p95 ceiling catches a systemic slowdown without flaking on one outlier — not a silent drop of the gate. The dominant cost is the shared engine turn; per-channel transport is a thin wrapper over it, so the gate is a regression alarm on the common driver, not a per-channel transport micro-benchmark. The budgets remain *targets* a further amendment can revise if a future change makes one genuinely infeasible on CI hardware.
...
5. **Sub-agent spawn.** Ask the main agent to "use a researcher sub-agent to find X". Expected: a new `researcher` agent is spawned via `AgentRegistry.spawn`, runs its own turn, returns; the trace contains the `forvum.graph.run` span with the spawned worker's `forvum.llm.call` span nested under it (OTel context propagated across the worker virtual-thread fan-out, P2-15).
