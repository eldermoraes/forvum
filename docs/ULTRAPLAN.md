...
**Post-v0.5 context-engineering differentiators (filed 2026-06-27).** Beyond the original ten, a post-v0.5 OpenClaw-parity + Context-Engineering audit surfaced three further v1.0+ bets, each sharpening one pillar to where `CONTEXT-ENGINEERING.md` currently out-aims the as-built runtime. Filed as tracking issues (not yet scheduled); not part of the original P3-1…P3-10.

11. **Real per-turn CAPR judge (#195).** Replace the placeholder `passed=1` verdict (item 4) with a genuine LLM-as-judge — reusing the offline eval machinery (`LlmJudge`/`MatcherJudge`), off the critical path and off by default — plus the `capr_events.model` column `ModelHealthReader` anticipates, so routing reflects answer quality, not just call health. Closes the success-metrics loop. *(CAPR pillar; cf. Risk #10.)*
12. **Iterative / agentic RAG (#196, blocked on #175).** Upgrade Select from single-shot `<retrieved_memory>` framing to a bounded retrieve → evaluate → decompose loop, optionally as an Isolate-respecting memory sub-agent (only a compressed digest crosses `reduce`) — the `CONTEXT-ENGINEERING.md` "Agentic RAG dinâmico" ask. *(Select pillar.)*
13. **Mid-turn context pruning (#197).** Add within-turn pruning of oversized tool results and out-of-window images (tail-first, prompt-cache-stable), complementing the between-turn `SessionCompactor` and the item-8 proxy-compression middleware. *(Compress pillar.)*
