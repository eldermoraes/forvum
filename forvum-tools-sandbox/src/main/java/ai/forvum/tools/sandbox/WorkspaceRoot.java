...
/**
 * The directory a {@code sandbox.run} working directory is confined to and mounted into the container
 * (ULTRAPLAN §9.2.5 / #27). A Layer-3 plugin cannot depend on another plugin, so this is the sandbox
 * module's own self-contained confinement seam (the same hardened two-stage approach as
 * {@code forvum-tools-shell}/{@code forvum-tools-filesystem}'s {@code WorkspaceRoot}, not promoted to a
 * shared layer per the ratified PR-6 decision).
 *
 ...
 * symlinked out of the root). The working directory MUST exist (the container bind-mounts it), so unlike a
 * filesystem write path there is no not-yet-existing case.
 *
...
public final class WorkspaceRoot {
     ...
     *
     * @return the absolute, link-resolved working directory to bind-mount into the container
    ...
    public Path confine(String workingDir) {
        ...
        if (!Files.exists(lexical, LinkOption.NOFOLLOW_LINKS)) {
            // A non-existent working directory cannot have escaped via a link; the container bind-mount
    ...
    private static Path realPath(Path path) {
        try {
            ...
            throw new UncheckedIOException(
                    "Cannot canonicalize working directory '" + path + "' for sandbox confinement.", e);
        }
    }
}
