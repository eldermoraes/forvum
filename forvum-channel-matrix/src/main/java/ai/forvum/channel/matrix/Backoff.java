...
/**
 * A pure, lock-free exponential-backoff counter for the Matrix sync-loop retry: each
 * {@link #nextDelayMillis()} doubles the delay (starting at {@code initialMillis}) up to a {@code maxMillis}
 * cap, and {@link #reset()} returns to the initial delay (called on a successful sync so a healthy loop
 * restarts the schedule). No clock, no IO, no {@code synchronized} — backed by a single {@link AtomicLong}
 * (CLAUDE.md §3.8). Computation is deterministic, so the schedule is unit-testable directly without
 * sleeping. Mirrors the Discord channel's {@code Backoff} (module isolation forbids a shared type).
 */
final class Backoff {
    ...
    /** Return the schedule to its initial delay (called on a successful sync). */
    void reset() {
