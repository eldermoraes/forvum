...
/**
 * Discovery marker for the Slack channel (P2-CH). Per the M16 SPI decision,
 * {@link ai.forvum.sdk.ChannelProvider} stays a pure discovery marker ({@code extensionId()} only) — the
 * channel is self-driving: its Socket Mode client ({@link SlackChannel} → {@link SlackSocketEndpoint})
 ...
 * manifest so the build-time plugin scanner records the extension; {@code @ApplicationScoped} makes it
 * CDI-discoverable.
 */
@ForvumExtension
@ApplicationScoped
public class SlackChannelProvider extends AbstractChannelProvider {
