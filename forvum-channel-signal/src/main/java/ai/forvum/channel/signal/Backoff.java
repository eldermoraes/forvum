...
/**
 * A pure, lock-free exponential-backoff counter for the Signal SSE reconnect loop: each
 * {@link #nextDelayMillis()} doubles the delay (starting at {@code initialMillis}) up to a {@code maxMillis}
 * cap, and {@link #reset()} returns to the initial delay (called on the first healthy event so a recovered
 * stream restarts the schedule). No clock, no IO, no {@code synchronized} — backed by a single
 * {@link AtomicLong} so the consuming virtual thread (event → reset) and the reconnect virtual thread
 * (next delay) can touch it without a lock (CLAUDE.md §3.8). Computation is deterministic, so the schedule
 * is unit-testable directly without sleeping. Mirrors the Discord channel's {@code Backoff} (module
...
final class Backoff {
    ...
    /** Return the schedule to its initial delay (called on the first healthy event after a (re)connect). */
    void reset() {
