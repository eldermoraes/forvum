 ...
 * which a Layer-3 channel must not depend on, so an {@code IT} here cannot reach it; this fake emits
 * Option B's shape — one {@link TokenDelta} echoing the inbound content, then a terminal {@link Done} —
 * and records every dispatched {@link ChannelMessage} so the test can assert the mapping ran (or, for a
 * refused/ignored message, that no turn was dispatched). Mirrors the Telegram/Discord channels' fakes.
 */
@ApplicationScoped
