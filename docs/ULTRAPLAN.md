...
- **`skills/<skill>.md`** is a named prompt template with an optional leading `---`-fenced **JSON** front-matter (`{ "name", "description", "inputSchema" }`) declaring its input schema — the same v0.5-subset `OutputSchemaValidator` shape used for agent output schemas (one validator, no parallel schema); a skill with no front-matter is valid (the whole file is the template). `forvum skill install <url>` (P2-7 #32) downloads a skill from a URL (http/https/file — a git-raw URL or gist), validates it through the real `SkillReader` parser (a malformed front-matter or `inputSchema` is rejected before the file lands — the doctor-lesson "validate through the real reader"), and writes it owner-only (`0600`); `skills/` is hot-loaded (`ConfigWatcher`). Agents invoke skills by name through the `SkillInvokerTool` (skills ARE tools; the surface is owned by M13 acceptance — X7), which validates invocation arguments against the declared `inputSchema` before expansion. Skills are globally visible to any agent allowed to call the skill tool — they are not per-agent.
