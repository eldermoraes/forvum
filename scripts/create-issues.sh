...
**Context.** First messaging channel (§7.1 M17). Tied to Risk #8 (long-poll vs webhook) and Risk #12 (reactive client -> resolved by a blocking client on a virtual thread, §3.8).

**Scope / Deliverables.** forvum-channel-telegram: long-poll bot via a blocking quarkus-rest-client on a virtual thread (@RunOnVirtualThread); webhook opt-in; allowedUserIds gate.

**Files.** `forvum-channel-telegram/.../TelegramChannel.java`, `TelegramBotApi.java` (blocking REST client), manifest.

**Acceptance Criteria.**
- With a bot token in the keychain, a live DM produces a reply within the turn-latency budget; allowedUserIds refuses other users with a friendly message. No Mutiny in channel/engine source -- the blocking client keeps the path virtual-thread-native (Risk #12); the M5/M6 import-grep guards against reactive creep.
...
- [PLUGIN] quarkus/skills for rest-client; context7 for Telegram Bot API shapes; add the extension via the MCP.
