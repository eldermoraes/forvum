...
|---|---|
| **Computational cost vs. latency** | The `CostBudget` contract (§4.3.5.2) treats cost and tokens as first-class architectural concerns. Per-agent and per-cron budgets opt in per dimension (USD-only, tokens-only, or both), with hard-stop enforcement via `BudgetExhaustedException` (**`[shipped]`** #169: the Decision-8 pre-call gate fires on every provider attempt, the per-turn `toolBudget` counter at the tool-execution boundary, and cron fires resolve budget-gated). Cost-driven exhaustion short-circuits the fallback chain (§5.4) instead of cascading through expensive retries. |
| **"Context Rot" and position bias** | `MemoryPolicy` (§4.3.6) and the three memory tiers (§4.2) keep the working context small and relevant (**`[shipped]`** #175: the bundled local SQLite `LocalMemoryProvider` writes durable facts + real episodes off-turn and reads them back per-turn, so a default install exercises the full three-tier loop without Qdrant). The design separates short-term conversational memory (`messages`) from episodic (`episodic_memory`) and semantic (`semantic_memory`) layers, each read back on the next turn under its own retrieval scope — keeping each layer small to avoid the "lost in the middle" problem. |
...
The three memory tiers are the Write pillar's tiered scratchpad surface (§4.2): reasoning state never depends solely on the prompt window. The design writes to the appropriate tier between turns and reads from it on the next turn, governed by the agent's `MemoryPolicy` (§4.3.6). **`[shipped]`** #175: a default install now runs the full loop — after each turn `MemoryWriter` extracts durable facts with the small model, runs them through the pre-memory-write secret filter, embeds them, and upserts `semantic_memory` (off-turn), while `recordTurn` writes the real turn content to `episodic_memory`; the bundled local `LocalMemoryProvider` reads both tiers back on the next turn (semantic cosine + episodic keyword), all identity/agent-scoped and Qdrant-free. **`[shipped]`** #193: a model-callable `memory.save` (MEMORY_WRITE) / `memory.recall` (MEMORY_READ) tool (`forvum-tools-memory`) layers a *deliberate* mid-turn write/read surface on the same contract via the `forvum-sdk` `MemoryAccess` seam — routed through the same DR-6a pre-memory-write filter, identity/agent-scoped, reusing `SemanticMemoryStore` + `MemorySelector` with no second index. **`[shipped]`** #190: the always-available `update_plan` built-in gives the model a *structured* scratchpad in the same `messages` tier — each call replaces the plan as one append-only rendered-checklist row (`block_type='plan'`), the live plan is re-injected into the window at every turn entry as a framed data block, and compaction retains the newest plan verbatim while stripping superseded ones. User-editable files under `~/.forvum/` carry the complementary Write surface users edit directly (§4.1, §2.7).

### Select — high-precision filtering

- `MemoryProvider` SPI (`forvum-sdk`, §2.2) lets implementations choose vector, graph, metadata, or hybrid retrieval without coupling the agent to a strategy. **`[shipped]`** #175: the bundled local `LocalMemoryProvider` is the default retrieval path wired into every turn (embed-the-cue cosine over `semantic_memory` + episodic keyword recall), and `MemorySelector` prefers a configured external provider (Qdrant) only when active — so Qdrant is opt-in, not a prerequisite. **`[shipped]`** #196: iterative/agentic RAG — a new `RetrievalStrategy.ITERATIVE` (OPT-IN; the default stays single-shot) swaps the turn-entry retrieve for `IterativeRetrieval`, a BOUNDED retrieve → evaluate-sufficiency → decompose/re-query loop (≤3 iterations, ≤3 sub-queries each) run as a blocking memory sub-agent on the turn's model whose evaluator transcript never enters the supervisor window (Isolate preserved — only the accumulated hits cross back, then compressed + `<retrieved_memory>` DATA-framed exactly like the single-shot path); ITERATIVE is an orchestration strategy layered ABOVE the SPI (each provider call runs the HYBRID blend), evaluator failure degrades gracefully to what was already retrieved, and the plain-text verdict protocol keeps it reflection-free/native-clean.
...
- Tool filtering (§5.3): the agent's `AgentToolBelt` intersects the global `ToolRegistry` against the agent's `allowedTools` globs, so the LLM sees only the relevant subset — Select applied to capability. **`[shipped]`** #185: the belt now includes a multimodal *input* surface — `image.analyze` / `pdf.analyze` (`forvum-tools-multimodal`, `MEDIA_ANALYZE`) let the agent pull a workspace image or PDF into the context via a vision-capable sub-generation (through the `forvum-sdk` `MediaAnalysis` seam, budget-gated and ledgered like any model call), so Select extends beyond text tools to the media the model can bring into a turn.
- Skills as on-demand procedural knowledge (§4.1): the model pulls a named `skills/<id>.md` prompt template into the window via `skill.invoke` (discovering the installed set via `skill.list`), arguments schema-validated against the skill's `inputSchema` before a `{{key}}` expansion — Select applied to procedure, so a large body of instructions stays out of the system prompt until a turn actually needs it. **`[shipped]`** #191: the engine-resident `SkillToolProvider`, belt-gated by `SKILL_INVOKE`, read at invoke time (hot-reloaded, no cache) and capped at 32 000 chars; the expanded content runs under the caller's own belt/scopes (containment, §9.1.c), so a skill escalates nothing.
...
- **Write-time summarization (§3.3, §5.5).** Tool results and retrieved memory whose serialized size exceeds the agent's `MemoryPolicy` threshold are summarized through the small-and-fast model (default Ollama `qwen3:1.7b`) before re-entering the context window; only the compressed digest is persisted. This applies at every write-back from the MVP (the `reduce` node, §5.5), not only as the Phase-3 proxy-model middleware (§7.3 item 8). **`[shipped]`** the summarization path ships, and a compression *failure* now degrades to a DETERMINISTIC bounded fallback (truncate to the boundary + a fixed marker, never reinserting raw text) via the shared `BoundedCompressor` policy at both the retrieved-memory and worker-digest boundaries (#176); within-turn pruning of oversized tool results/images is **`[shipped]`** #197: `MidTurnPruner` runs before every model call in the supervisor graph, in-place past the frozen-prefix boundary (prompt-cache-stable — never insert/remove/reorder), middle-eliding oversized tool results, replacing stale out-of-window images with a marker, and stripping superseded thinking payloads; governed by the same `MemoryPolicy.compressThresholdChars` knob (≤0 disables the pass, preserving replay determinism).
...
- **`@AgentScoped` custom CDI context** (§5.1) — every per-agent bean (`AgentMemory`, `AgentToolBelt`, `AgentChatModel`, `AgentGraph`) resolves to instances keyed by the active `AgentId`. Two concurrent agents see two independent universes of beans. The context is an ArC `InjectableContext` registered at build time so it survives native compilation. **`[shipped]`** (#177) — ephemeral spawned workers are retired at turn end: the supervisor's `run()` `finally` calls `AgentRegistry.retire`, which unregisters the worker (freeing its registry slot) and destroys its `@AgentScoped` context through the ArC-registered `AgentContext`, so a long-running server stays bounded. Each worker gets a unique runtime id (`EphemeralAgentId`, `<label>~<uuid8>`) so a spawn can never collide with a persistent agent, and ledger/task history survives teardown.
...
| **Operational metrics** | `provider_calls` records latency per call, and the `Usage` snapshot (§4.3.5.2) exposes spent/remaining at any point. OpenTelemetry spans (§3.6) add throughput and end-to-end latency. The `BudgetExhaustedException` flow (§4.3.5.2, §5.4) delivers cost-aware degradation (**`[shipped]`** #169: the pre-call hard stop is enforced at runtime, surfaced as a terminal `budget_exhausted` error). |
