 ...
 * fake emits Option B's shape — one {@link TokenDelta} echoing the inbound (transcribed) content, then a
 * terminal {@link Done} — and records every dispatched {@link ChannelMessage} so the test can assert that
 * the STT→turn mapping ran. Live channel-to-engine wiring against real whisper/piper is a
 * {@code *LiveTest @Tag("live")} concern, default-off.
 */
@ApplicationScoped
